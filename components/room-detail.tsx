...
import ReserverForm from "@/components/reserve-form";
  ...
  return (
    <div className="max-w-screen-xl py-16 px-4 grid lg:grid-cols-12 gap-8 mx-auto">
      <div className="md:col-span-8">
        ...
        />
        <h1 className="text-5xl font-semibold text-gray-900 mb0-8">
          {room.name}
        </h1>
        ...
        <div className="grid md:grid-cols-3">
          {room.RoomAmenities.map((item) => (
            <div className="flex gap-1 py-1" key={item.Amenities.name}>
      ...
      <div className="md:col-span-4">
        <div className="border-2 border-gray-300 border-dashed px-3 py-5 bg-slate-50 rounded-md">
          <div className="flex itmes-center justify-between  mb-8">
          ...
          {/* reservation form */}
          <ReserverForm room={room} disabledDate={disabledDate} />
        </div>
      </div>
    </div>
  );
